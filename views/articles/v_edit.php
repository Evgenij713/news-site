...
<form class="appForm" method="post">
	...
	<div class="form-group">
		...
		<input type="text" name="title" id="title" class="form-control" size="100" value="<?=$article['title']?>">
	</div>
	<div class="form-group">
		<label for="content">Текст:</label>
		<textarea rows="10" cols="100" name="content" id="content" class="form-control"><?=$article['content']?></textarea>
	</div>
	<div class="form-group">
		<label for="category">Категория:</label>
		<select name="category" id="category" class="form-control">
			<?php foreach($categories as $category): ?>
				<option value="<?=$category['id_category']?>" <?=($category['id_category'] === $article['id_category']) ? 'selected' : '' ?>><?=$category['name']?></option>
			<?php endforeach; ?>
		</select>
	</div>
	<div class="form-group">
